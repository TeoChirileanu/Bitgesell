...
describe('Items Router', () => {
  ...
  describe('GET /api/items', () => {
    test('should return all items', async () => {
      ...
      expect(response.body).toEqual(testItems);
    });
    ...
    test('should return filtered items when search query is provided', async () => {
      ...
      expect(response.body[0].name).toBe(testItems[0].name);
    });
    ...
    test('should limit the number of items returned when limit is provided', async () => {
      ...
      expect(response.body[0].id).toBe(testItems[0].id);
      expect(response.body[1].id).toBe(testItems[1].id);
    });
